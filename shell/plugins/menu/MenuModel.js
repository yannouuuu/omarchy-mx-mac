...
function guardHelpers() {
  return 'declare -A __omarchy_pkgs=()\n'
    ...
    + 'declare -A __omarchy_sync_pkgs=() __omarchy_install_pkgs=()\n'
    ...
    + 'done < "$OMARCHY_PATH/install/optional-packages.tsv"\n'
    + 'omarchy-install-available() { local packages=${__omarchy_install_pkgs[$1]-} p; '
    + '[[ -n $packages ]] || return 1; read -ra packages <<<"$packages"; '
    + 'for p in "${packages[@]}"; do __omarchy_sync_has "$p" || return 1; done; return 0; }\n'
